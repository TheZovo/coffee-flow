...
async def run_startup_migrations(conn: AsyncConnection) -> None:
    ...
    await conn.execute(
        text(
            ...
            SET bonus_redeem_max_percent = 100
            WHERE bonus_redeem_max_percent IS NULL
            """
        )
    )
    await conn.execute(
        text(
            """
            ALTER TABLE loyalty_settings
            ALTER COLUMN bonus_redeem_max_percent SET DEFAULT 100
            """
        )
    )
    await conn.execute(
    ...
    # FIX: ADD COLUMN для NOT NULL колонок выполняется ДО INSERT,
    # чтобы при вставке строки все колонки уже имели DEFAULT значения.
    await conn.execute(
        text(
            """
    ...
    # Явно выставляем DEFAULT — на случай если колонки уже существовали без него
    # (ADD COLUMN IF NOT EXISTS полностью пропускает уже существующие колонки)
    await conn.execute(
        text("ALTER TABLE app_settings ALTER COLUMN inactive_reminder_enabled SET DEFAULT FALSE")
    )
    await conn.execute(
        text("ALTER TABLE app_settings ALTER COLUMN inactive_reminder_days SET DEFAULT 30")
    )
    await conn.execute(
        text("ALTER TABLE app_settings ALTER COLUMN inactive_reminder_send_time SET DEFAULT '12:00'")
    )
    # INSERT явно передаёт значения NOT NULL колонок, не полагаясь на DEFAULT в каталоге
    await conn.execute(
        text(
            """
            INSERT INTO app_settings (id, inactive_reminder_enabled, inactive_reminder_days, inactive_reminder_send_time)
            VALUES (1, FALSE, 30, '12:00')
            ON CONFLICT (id) DO NOTHING
            """
        )
    )
